...
# 注入 JavaScript 達成自動對焦 (Autofocus)
components.html(
    """
    <script>
    window.parent.document.querySelectorAll('input[type="number"]')[0].focus();
    </script>
    """,
    height=0,
)
...
if buy_p:
    ...
    with res_box:
        ...
        # 建立表格數據
        data = []
        up_prices = []; curr_up = buy_p
        for _ in range(5):
            curr_up += get_tick(curr_up); up_prices.append(curr_up)
        
        down_prices = []; curr_down = buy_p
        for _ in range(5):
            curr_down -= get_tick(curr_down - 0.01); down_prices.append(curr_down)
        
        all_p = up_prices[::-1] + [buy_p] + down_prices
        
        for p in all_p:
            s_total = p * qty * 1000
            s_fee = int(s_total * 0.001425 * current_disc); tax = int(s_total * 0.0015)
            net = int(s_total - s_fee - tax - total_cost)
            
            diff = 0
            if p > buy_p:
                tmp = buy_p
                while tmp < p: tmp += get_tick(tmp); diff += 1
            elif p < buy_p:
                tmp = buy_p
                while tmp > p: tmp -= get_tick(tmp - 0.01); diff -= 1
            ...
            data.append({
                "跳動": label, "賣出價": f"{p:.2f}", 
                "實際盈虧": net, "報酬%": f"{(net/total_cost)*100:.2f}% {trend}"
            })
        
        st.dataframe(
            pd.DataFrame(data), 
            column_config={"實際盈虧": st.column_config.NumberColumn("實際損益", format="%d")}, 
            hide_index=True, 
